#include <object_detect.h>
  ...
  void BlockDetect::cloud_cb(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input)
  {
      ...
      pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> color(this->curr_cloud);
      this->viewer->addPointCloud<pcl::PointXYZRGB> (this->curr_cloud, color, "sample cloud");
  ...
  bool BlockDetect::srv_cb(yumi_block_detection::objectLocation::Request  &req,
         yumi_block_detection::objectLocation::Response &res)
  {
    this->filename = req.filename;
    res.objectLocation = this->startRoutine();
    return true;
  }
  ...
  PointCloud::Ptr BlockDetect::removeGroundPlane()
  {
  ...
  */
  ...
  yumi_block_detection::objects BlockDetect::startRoutine()
  {
    while (true) {
      if ((this->curr_cloud)->points.size() > 0)
      {
        PointCloud::Ptr filCloud = this->removeGroundPlane();
        this->curr_cloud = filCloud;

        /*
          Note: The topmost block in patttern should be in at 0th index
          of the array and lowest block at the last element position.
        ...
        */
        yumi_block_detection::objectCentroid data;
        yumi_block_detection::objects msg;


        for (int k = 0; k < (sizeof pattern / sizeof pattern[0]); k++)
        {
        ...
        std::cerr << "if end ";
        std::cerr << "size: " << msg.objectArray.size();
        return (msg);
      }
      else
        ros::spinOnce();
  ...
  int main (int argc, char** argv)
  {
    ...
    ROS_INFO_STREAM_NAMED("BlockDetect", ">> Waiting for data from PrimeSense...");

    sub = detector.nh.subscribe ("/camera/depth_registered/points", 1, &BlockDetect::cloud_cb, &detector);
    service = detector.nh.advertiseService ("/yumi_object_location", &BlockDetect::srv_cb, &detector);
    ros::spin ();
